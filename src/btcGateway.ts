...
export function handleLogMint(event: LogMint): void {
  ...
  integrator.totalTxCountBTC = integrator.totalTxCountBTC.plus(oneBigInt());
  integrator.totalVolumeBTC = integrator.totalVolumeBTC.plus(tx.amount);
  integrator.totalLockedBTC = integrator.totalLockedBTC.plus(tx.amount);
  integrator.save();
